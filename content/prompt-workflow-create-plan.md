...
type: prompt
subtype: workflow
tags: [planning, implementation, tdd, spec-driven, architecture]
...
Use this prompt when you need to create a detailed implementation plan before starting development.

**Critical for:**
- Complex features requiring multiple steps or phases
- Features that need architectural decisions
- Work that will span multiple files or components
- Changes with multiple valid implementation approaches
- Features requiring Test-Driven Development planning

**Do NOT use when:**
- Making trivial changes (typos, simple bug fixes)
- Requirements are too vague to plan (do research first)
- Prototyping or exploring (spike first, then plan)
...
**Input:**
```
Create a detailed implementation plan for adding dark mode support to the web application. The feature should persist user preference and default to system preference.
```

**AI Response (following prompt):**
```
I'll create an implementation plan for dark mode support. Let me research the codebase first to understand the current styling approach and identify what needs to change.

[Creates todo list for research:]
- [ ] Find current styling implementation (CSS modules, styled-components, etc.)
- [ ] Locate theme-related code if any
- [ ] Check for existing color variables/design tokens
- [ ] Identify state management patterns
- [ ] Find local storage usage patterns
...
- Local storage used in utils/storage.ts
- React Context for global state

I'll create a 4-phase plan that builds incrementally:

Phase 1: Theme infrastructure (Context + storage)
Phase 2: CSS variable system for colors
Phase 3: Dark mode styles for components
Phase 4: System preference detection

[Writes plan to plans/2026-01-12-dark-mode-support.md]

Plan created at plans/2026-01-12-dark-mode-support.md

**Summary:**
- 4 phases, each independently verifiable
- Follows TDD with tests for theme switching, persistence, and system detection
- Estimated 6-8 hours total (1-2 hours per phase)
- Uses existing patterns (Context API, localStorage)

**Key decisions:**
- CSS variables for colors (enables runtime switching without CSS-in-JS overhead)
- System preference as default (better UX)
- React Context for theme state (consistent with auth pattern)

**Risks identified:**
- Flash of wrong theme on page load (mitigated with inline script)
- Third-party components may not support dark mode (mitigated by wrapper components)

Would you like me to adjust anything in the plan?
```

## Expected Results

- A detailed, actionable implementation plan saved to `plans/` directory
- Clear phases with specific file changes and test requirements
- Success criteria for each phase (automated and manual)
- All questions resolved before finalizing
- Plan ready for review or immediate implementation

## Variations

**For Small Features (<100 LOC estimated):**
```markdown
Create a lightweight plan with:
- Single phase instead of multiple
...
**For Exploratory/Spike Work:**
```markdown
Create a research plan instead:
- Goals: What we're trying to learn
- Experiments: What we'll try
- Success metrics: How we'll know if it's viable
- Decision point: When we'll decide to proceed or pivot
```

**When Working with Issue Trackers:**
```markdown
Reference issue numbers in plan:
- Link plan sections to issue IDs
- Create sub-tasks from phases
- Update issue descriptions with plan reference
```

## References

- [Test-Driven Development by Kent Beck](https://www.oreilly.com/library/view/test-driven-development/0321146530/)
- `prompt-workflow-plan-implement-verify-tdd.md` - Full TDD workflow
- `prompt-workflow-iterate-plan.md` - How to update existing plans
- `prompt-task-plan-review.md` - Quality review for plans
...
**Benefits:**
- **Clarity**: Thinking through the problem before coding
- **Alignment**: Get user buy-in on approach
- **Estimation**: Understand scope and time required
- **Checkpoints**: Break work into verifiable phases
- **Documentation**: Record decisions and reasoning

**When to skip:**
- Trivial changes where the approach is obvious
- Prototyping/spikes (document after if continuing)
- Emergency hotfixes (document in retrospective)

### Plan Quality Indicators

A good plan should:
- Be specific enough to start implementing immediately
- Have clear, testable success criteria for each phase
- Include estimated time for each phase
- Identify risks and mitigation strategies
- Reference existing patterns and conventions
- Be readable by another developer/agent

A bad plan:
- Uses vague language ("improve", "update", "fix")
- Lacks concrete file paths
- Has no test strategy
- Doesn't break work into phases
- Leaves open questions unresolved

### Iterating on Plans

Plans are living documents:
- Mark phases complete as you finish them (checkboxes)
- Update if reality doesn't match expectations
- Add learnings as you discover them
- Reference the plan in commits and PRs

Use `prompt-workflow-iterate-plan.md` to formally update a plan.

### Working with Multiple Agents/Sessions

Plans enable handoffs:
- Another agent can pick up where you left off
- Phase boundaries are natural handoff points
- Success criteria verify work without context
- Plans reduce context needed to contribute
