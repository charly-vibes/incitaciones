...
- **content/distilled/codebase-cartography/SKILL.md** — Zoom Discipline: batch-request carve-out for the offer-don't-dump rule; re-verify counts/fan-in numbers against a fresh command run before writing each report. Reference protocol names the Batch export section for multi-report requests.
